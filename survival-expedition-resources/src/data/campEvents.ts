...
const TEMPLATES: EventTemplate[] = [
  ...
  { minSurvivors: 2, generate: ([s1, s2]) => `${s1.name} fait tomber quelque chose. ${s2.name} lève les yeux. ${s1.name} dit "c'est bon". ${s2.name} se rebaisse.` },
  ...
  { minSurvivors: 2, generate: ([s1, s2]) => `${s1.name} et ${s2.name} décident de compter jusqu'à cent dans leur tête en même temps. Résultats différents.` },
